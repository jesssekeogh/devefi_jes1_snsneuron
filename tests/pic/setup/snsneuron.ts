...
export class SnsNeuron {
  ...
  public async splitNeuron(
    ...
    const { command }: ManageNeuronResponse =
      await this.snsGovActor.manage_neuron({
        ...
        command: [
          {
            Split: { memo: nonce, amount_e8s: amount_e8s },
          },
        ],
      });
    ...
    if (
      !("Split" in command[0]) ||
      !("created_neuron_id" in command[0].Split)
    ) {
      throw new Error("Failed to split neuron");
    }

    const { id } = command[0].Split.created_neuron_id[0];
  ...
  public async addPermission(
        ...
        command: [
          {
            AddNeuronPermissions: {
              permissions_to_add: [{ permissions: [permisson] }],
    ...
    if (!("AddNeuronPermission" in command[0])) {
