            ...
            func(t : Ver1.Mem) : Mem {
                {
                    main = Map.map(
                        t.main,
                        Map.n32hash,
                        func(_nodeId : Nat32, nodeMem : Ver1.SnsNodeMem) : SnsNodeMem {
                            {
                                variables = {
                                ...
                                internals = {
                                    ...
                                    var neuron_claimed = nodeMem.internals.neuron_claimed;
                                    var neuron_state = nodeMem.internals.neuron_state;
                                    var governance_canister = nodeMem.internals.governance_canister;
                                };
                                var neuron_cache = null; // will update by itself
                                var parameters_cache = nodeMem.parameters_cache;
                                var log = nodeMem.log;
                            };
                        },
                    );
        ...
        topic_followees : ?{
            topic_id_to_followees : [(
                Int32,
                {
                    ...
                    followees : [{ alias : ?Text; neuron_id : ?{ id : Blob } }];
                },
            )];
        };
